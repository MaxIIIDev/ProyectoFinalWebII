...
_a = CamaService;
CamaService.buscarCama = async (id_Cama) => {
    try {
        if (!id_Cama) {
        ...
        const cama = await Hospital_camas_1.Hospital_camas.findOne({
            include: [
                {
...
CamaService.buscarGeneroDeCamaOcupadaPorHabitacion = async (id_habitacion) => {
    try {
        if (!id_habitacion) {
...
CamaService.marcarCamaComoOcupada = async (id_Cama) => {
    //*TESTEADO
    try {
        const camaEncontrada = await _a.buscarCama(id_Cama);
        if (camaEncontrada[0]) {
        ...
        const [filasActualizadas] = await Hospital_camas_1.Hospital_camas.update({ disponible: false }, { where: { id_Cama } });
...
CamaService.marcarCamaComoLibre = async (id_Cama) => {
    //*TESTEADO
    try {
        const camaEncontrada = await _a.buscarCama(id_Cama);
        if (camaEncontrada[0]) {
        ...
        const [filasActualizadas] = await Hospital_camas_1.Hospital_camas.update({ disponible: true }, { where: { id_Cama } });
        if (filasActualizadas === 0) {
